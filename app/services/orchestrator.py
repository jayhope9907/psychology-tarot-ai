...
def _should_offer_assessment(state: ChatSessionState, user_message: str) -> bool:
    return detect_counseling_request(user_message) or detect_distress(user_message) or state.turn_count >= 3
...
def decide_turn(
    ...
    if should_block_new_assessment(state, user_message):
        reason = "warmup" if state.turn_count < 2 else "fatigue_or_spacing"
        if fatigue["blocked"]:
    ...
    reason = "ai_matched_assessment" if selection.method == "ai" else "rule_matched_assessment"
